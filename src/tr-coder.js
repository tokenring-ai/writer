...
import {SQLiteChatHistoryStorage, SQLiteChatCheckpointStorage, SQLiteChatMessageStorage} from "@token-ring/sqlite-storage";
...
.option('-i, --initialize', 'Initialize the source directory with a new config directory')
.addHelpText('after', `
Examples:
...
async function runCoder({ source, config: configFile, initialize }) {
 ...
 const db = initializeLocalDatabase(path.resolve(configDirectory, "./coder-database.sqlite"));
...
/*
 ...
 for (const resourceName in config.resources ?? {}) {
  ...
  await registry.resources.addResource(resourceName,
